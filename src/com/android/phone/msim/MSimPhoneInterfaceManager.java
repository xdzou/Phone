...
public class MSimPhoneInterfaceManager extends ITelephonyMSim.Stub {
    ...
    public boolean isDataConnectivityPossible() {
        return getPhone(((MSimPhoneGlobals)mApp).getDataSubscription()).isDataConnectivityPossible();
    }
    ...
    public String getNetworkName(int subscription) {
        ...
        ServiceState serviceState = getPhone(subscription).getServiceState();
        if (serviceState != null && serviceState.getState() == ServiceState.STATE_IN_SERVICE) {
            ...
            //init network name to "unknown network"
            ...
            if (isNumericValid) {
                ...
                int phoneType = getActivePhoneType(subscription);
                if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                    //if mcc+mnc is invalid, for cdma, get system network id.
                    CdmaCellLocation cdmaCellLocation = (CdmaCellLocation)MSimTelephonyManager.getDefault()
                                                        .getCellLocation(subscription);
