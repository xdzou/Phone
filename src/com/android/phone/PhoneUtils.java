...
public static boolean IsInBlackList(Phone phone,String number){
	...
	{
		  if (DBG) log(" IsInBlackList, firewall is not enable!");
		 return false;			 
	}
		 ...
		 if(number != null && !number.equals("")){
			 ...
			 if(isFirewallEnabled(context)){
				 switch (Integer.valueOf(getFirewallMode(context))) {
					 case 0: // blacklist
					 {
						 if (DBG) log(" isForbiddenNumber, blacklist is enable!");
						 int len = number.length();
						 if(len > 11)
						 {
							 number = number.substring(len - 11, len);
					 ...
					 case 1: // whitelist
					 {
						 if (DBG) log(" isForbiddenNumber, white is enable!");
						 int len = number.length();
						 if(len > 11)
						 {
							 number = number.substring(len - 11, len);
					 ...
					 case 2: // block all
						 if (DBG) log(" isForbiddenNumber, block all mode is enable!");
						 return true;
						 //break;
						 
					 case 3: // only accept contacts
					 {
						 if (DBG) log(" isForbiddenNumber, only accept contacts mode is enable!");
